...
export function computeEasyMode(data, hasFundamentals) {
  ...
  // already computed upstream — no new data needed. Blending it in when it's *above* the level
  // signal gives real credit for a turn already underway; when it's below (YELP: latest 3.7% is
  // still under its own 5.6% trailing median — still decelerating, no turn), the level signal is
  // left alone.
  const isReaccelerating = growthCagrPct != null && data.revGrowth != null && data.revGrowth > growthCagrPct;
  const effectiveGrowthPct = isReaccelerating ? (growthCagrPct + data.revGrowth) / 2 : growthCagrPct;
