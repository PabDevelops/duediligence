...
// Traqcker Value: Fair P/E x EPS, where Fair P/E starts from a 15x market-average
// anchor and is adjusted by three independently-tiered multipliers — quality (ROIC vs.
// sector threshold, reusing computeEasyMode's threshold logic), growth (EPS CAGR,
// falling back to revenue growth), and leverage (debt/equity). Each multiplier has 5
// tiers ordered worst-to-best; bear/bull scenarios shift every available tier by one
// step instead of only flexing growth like the old Graham formula did. No external
// "market constant" (e.g. a bond yield) is used — every input is something the app
// already computes and can show its work for.
const BASE_PE = 15;
const QUALITY_MULTS = [0.80, 0.95, 1.05, 1.15, 1.30];
const GROWTH_MULTS = [0.70, 0.90, 1.00, 1.15, 1.35];
const LEVERAGE_MULTS = [0.70, 0.75, 0.90, 1.00, 1.05];
export const QUALITY_TIER_LABELS = ['Weak ROIC', 'Below-avg ROIC', 'Solid ROIC', 'Strong ROIC', 'Exceptional ROIC'];
export const GROWTH_TIER_LABELS = ['Shrinking', 'Flat', 'Modest growth', 'Strong growth', 'High growth'];
export const LEVERAGE_TIER_LABELS = ['Negative equity', 'High debt', 'Moderate debt', 'Low debt', 'Very low debt'];

function qualityTierIdx(roic, threshold) {
  if (roic == null) return null;
  const r = roic / 100;
  if (r < threshold * 0.5) return 0;
  if (r < threshold) return 1;
  if (r < threshold * 1.5) return 2;
  if (r < threshold * 2) return 3;
  return 4;
}
function growthTierIdx(g) {
...
function leverageTierIdx(d2e) {
  if (d2e == null) return null;
  if (d2e < 0) return 0;
  if (d2e >= 3) return 1;
  if (d2e >= 1.5) return 2;
  if (d2e >= 0.5) return 3;
...
export function computeTraqckerValue(data) {
  // A P/E-based estimate is meaningless on negative earnings — EPS x Fair P/E would
  // produce a negative "fair value" instead of correctly saying "no estimate here".
  if (!data.eps || data.eps <= 0) return null;

  const sector = (data.sector || '').toLowerCase();
  const isTech = sector.includes('tech') || sector.includes('software') || sector.includes('semi');
  const isPharma = sector.includes('pharma') || sector.includes('biotech') || sector.includes('health');
  const roicThreshold = isTech ? 0.25 : isPharma ? 0.20 : 0.15;
  const g = data.epsCagr !== null && !isNaN(data.epsCagr) ? data.epsCagr : data.revGrowth;

  const qIdx = qualityTierIdx(data.roic, roicThreshold);
  const gIdx = growthTierIdx(g);
  const lIdx = leverageTierIdx(data.debtToEquity);

  const shift = (idx, delta) => idx == null ? null : Math.max(0, Math.min(4, idx + delta));
  const multAt = (mults, idx) => idx == null ? 1 : mults[idx];

  const buildScenario = (key, label, delta, primary) => {
    const qIdxShifted = shift(qIdx, delta);
    const gIdxShifted = shift(gIdx, delta);
    const lIdxShifted = shift(lIdx, delta);
    const qualityMult = multAt(QUALITY_MULTS, qIdxShifted);
    const growthMult = multAt(GROWTH_MULTS, gIdxShifted);
    const leverageMult = multAt(LEVERAGE_MULTS, lIdxShifted);
    const rawPE = BASE_PE * qualityMult * growthMult * leverageMult;
    const fairPE = Math.max(6, Math.min(40, rawPE));
    const value = +(data.eps * fairPE).toFixed(2);
    return {
      key, label, primary, value, fairPE: +fairPE.toFixed(1),
      quality: qIdxShifted != null ? { idx: qIdxShifted, mult: qualityMult, label: QUALITY_TIER_LABELS[qIdxShifted] } : null,
      growth: gIdxShifted != null ? { idx: gIdxShifted, mult: growthMult, label: GROWTH_TIER_LABELS[gIdxShifted] } : null,
      leverage: lIdxShifted != null ? { idx: lIdxShifted, mult: leverageMult, label: LEVERAGE_TIER_LABELS[lIdxShifted] } : null,
    };
  };
  ...
  return {
    basePE: BASE_PE,
    scenarios: [
      buildScenario('bear', 'BEAR CASE', -1, false),
      buildScenario('base', 'BASE CASE', 0, true),
      buildScenario('bull', 'BULL CASE', 1, false),
    ],
  };
}
...
// Two-part WACC-discounted DCF: 10 years of FCF compounded at a flat growth rate,
// terminal value via exit P/FCF multiple (not Gordon perpetuity — matches the source
// spreadsheet), both discounted back at WACC. Bear/bull scenarios flex WACC and growth
// in opposite directions instead of only flexing one input.
export function computeDCFValue(data, riskFreeRate) {
  ...
  const buildScenario = (key, label, waccDelta, growthDelta, primary) => {
    ...
    for (let year = 1; year <= 10; year++) {
      fcf = fcf * (1 + scenarioGrowth);
      pvExplicit += fcf / Math.pow(1 + scenarioWacc, year);
    }
    const terminalValue = fcf * exitMultiple;
    const pvTerminal = terminalValue / Math.pow(1 + scenarioWacc, 10);
    const totalPV = pvExplicit + pvTerminal;
    const value = +(((totalPV + surplusCash) / data.sharesOutstanding)).toFixed(2);

    return { key, label, primary, value, wacc: scenarioWacc, growth: scenarioGrowth, terminalValue, totalPV };
  };

  return {
    wacc, baseGrowth, exitMultiple, surplusCash,
    scenarios: [
