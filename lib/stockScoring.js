...
// needs a growth rate that comes from the business, not from its own quote: FCF CAGR over
...
export function computeFundamentalGrowth(data) {
  const hist = (data.fcfHistory || []).filter(h => h.val > 0);
  if (hist.length >= 2) {
    const first = hist[0];
    ...
    if (years > 0 && first.val > 0) {
      ...
      if (isFinite(cagr)) return Math.max(-0.10, Math.min(0.25, cagr));
    }
  }
  if (data.revGrowth != null) return Math.max(-0.10, Math.min(0.25, data.revGrowth / 100));
  return 0.05;
}
...
// today's own pricing twice over). Same sector buckets used elsewhere in this file
// (computeEasyMode) for consistency; the multiples themselves are rough long-run
// benchmarks, not fitted to any individual ticker.
const SECTOR_EXIT_MULTIPLES = { tech: 24, pharma: 20, consumer: 18, financial: 13, energy: 11, default: 16 };
export function computeNormalizedExitMultiple(sector) {
  ...
  if (s.includes('retail') || s.includes('consumer') || s.includes('food') || s.includes('beverage')) return SECTOR_EXIT_MULTIPLES.consumer;
...
export function computeDCFPointValue(data, riskFreeRate, overrides = {}) {
  ...
  const exitMultiple = overrides.exitMultiple ?? computeNormalizedExitMultiple(data.sector);
...
export function computeDCFValue(data, riskFreeRate) {
  ...
  const exitMultiple = computeNormalizedExitMultiple(data.sector);
...
export function computeBlendedVolatility({ historicalVol, beta, marketVolAnnual }) {
  const betaMarketVol = beta != null && marketVolAnnual != null ? Math.abs(beta) * marketVolAnnual : null;
...
export function computeProjection({ price, driftAnnual, volAnnual, horizonYears, stepsPerYear = 52, rng = Math.random, numPaths = 18 }) {
  ...
  const drift = driftAnnual, vol = volAnnual;
  ...
  const paths = Array.from({ length: numPaths }, () => {
    ...
    for (let i = 1; i <= totalSteps; i++) {
      ...
      simPrice = simPrice * Math.exp((drift - 0.5 * vol * vol) * dt + vol * Math.sqrt(dt) * z);
      path.push(simPrice);
    }
    return path;
  });
  ...
  for (let i = 1; i <= totalSteps; i++) {
    const t = i * dt;
    const driftTerm = (drift - 0.5 * vol * vol) * t;
