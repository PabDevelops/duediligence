...
import { getCapTier, DEFAULT_TIER } from './marketCap';
...
// Coefficient of variation (stdev/mean) of a numeric series — flags cyclicality/instability
// that a single TTM snapshot can't see (see computeStabilityFactor below).
...
// Least-squares trend slope through (index, value) — sign tells direction, independent of the
// series' absolute level, used to catch a margin that's steadily eroding even if its CV is low.
...
// Historical stability of operating margin and ROIC over the trailing ~7 years (stub/macro-
// shock years excluded via excludedYears, same cleaning the DCF's own growth engine uses below)
// — without this, a cyclical business caught at its margin peak, or a value trap riding a
// temporary ROIC spike, scores identically to a genuinely stable compounder, since CBS otherwise
// only ever looks at the latest reported year. Returns a multiplier in (0.85, 1]: 1 when there
// isn't enough history to judge (never penalize on absence of data), scaling down toward 0.85
// as either series' CV clears the tier's CV threshold or the 5-year margin trend is clearly
// deteriorating. The threshold itself comes from the company's market-cap tier (see
// lib/marketCap.js): a micro-cap's earnings are structurally lumpier than a mega-cap's, so the
// same coefficient of variation that would flag real instability at mega-cap scale is just
// normal noise lower down — penalizing it identically would systematically punish every small/
// micro-cap regardless of actual business quality.
//
// isCyclicalCommodity (materials/metals/mining, energy) uncaps the discount and drops the floor
// to 0.5 — justified for a business whose margin/ROIC genuinely mean-reverts with an external
// commodity price. Every other sector keeps the original, gentler cap/floor (max 15% off,
// floor 0.85): high CV outside a price-cyclical sector usually means lumpy R&D, licensing
// income, or one-off charges, not "caught at an unsustainable peak" — verified against real
// data: INCY's marginHistory (-9.9%, 19.6%, 17.1%, 16.8%, 1.4%, 29.5%) carries a CV of 1.04
// purely from ordinary biotech earnings lumpiness (a 2024 one-off charge, then a genuine
// margin-scaling recovery to its *best* year yet in 2025), not a windfall about to reverse —
// the uncapped version misread that recovery as instability and cut CBS/OPPO/GQS by ~28% each,
// dragging an improving, profitable biotech's score from ~87 to 67.
function computeStabilityFactor(data, years = 7, capTier = DEFAULT_TIER, isCyclicalCommodity = false) {
  const revHist = data.revHistory || [];
  const bad = excludedYears(revHist);

  const marginSeries = (data.marginHistory || [])
    .filter(m => m.margin != null && !bad.has(m.year))
  ...
  const roicSeries = Object.keys(oi)
    .filter(y => !bad.has(y) && (debt[y] != null || equity[y] != null || data.debtVal != null || data.equityVal != null))
    .sort((a, b) => parseInt(a, 10) - parseInt(b, 10))
    .slice(-years)
    .map(y => {
      const capital = (debt[y] ?? data.debtVal ?? 0) + (equity[y] ?? data.equityVal ?? 0);
      return capital > 0 ? (oi[y] / capital) * 100 : null;
    })
    .filter(v => v != null);

  const marginCV = coefficientOfVariation(marginSeries);
  const roicCV = coefficientOfVariation(roicSeries);
  if (marginCV == null && roicCV == null) return { factor: 1, marginCV, roicCV };

  const marginSlope = trendSlope(marginSeries.slice(-5));
  const isDeclining = marginSlope != null && marginSlope < 0 && marginSeries.length >= 5
    && marginSeries[marginSeries.length - 1] < marginSeries[marginSeries.length - 5] * 0.9;

  const CV_THRESHOLD = capTier?.stabilityCvThreshold ?? DEFAULT_TIER.stabilityCvThreshold;
  const worstCV = Math.max(marginCV ?? 0, roicCV ?? 0);
  let factor = 1;
  // Uncapped discount + lower floor only for sectors where extreme CV is verified to mean
  // "riding a commodity price cycle" (see isCyclicalCommodity above) — a mildly-above-threshold
  // CV and a wildly cyclical one (a commodity miner's margin/ROIC on a price spike, CV well
  // over 1.0) otherwise got the identical, token 15% haircut. Verified against real data: HL's
  // marginCV/roicCV both topped 1.2 (over 4x the large-cap threshold) yet still landed at the
  // same floor as a stock barely past the threshold. Every other sector keeps the original cap
  // so ordinary earnings lumpiness (biotech R&D/licensing swings, one-off charges) doesn't get
  // mistaken for the same "unsustainable peak" problem.
  const discountCap = isCyclicalCommodity ? Infinity : 0.15;
  const floor = isCyclicalCommodity ? 0.5 : 0.85;
  if (worstCV > CV_THRESHOLD) factor -= Math.min(discountCap, (worstCV - CV_THRESHOLD) * 0.3);
  if (isDeclining) factor -= 0.05;
  return { factor: Math.max(floor, factor), marginCV, roicCV };
}

// A second, sector-agnostic way into the "cyclical commodity" treatment above: a genuine
// top-line crash — not just a margin swing — within the trailing history. isMaterials/isEnergy
// (sector name matching) misses memory-chip makers entirely: Finnhub/SEC classify MU under the
// same generic "Semiconductors" sector/industry as NVDA or TSM, with no field distinguishing a
// commodity-priced memory maker from a differentiated logic/design one. But MU's own revenue
// history tells the story sector-matching can't: 2022's $30.8B collapsed to $15.5B in 2023 (the
// DRAM/NAND price crash), then rebounded to $25.1B and $37.4B as AI-driven memory demand pushed
// prices back up — a -49.5% YoY drop, the signature of a commodity ASP cycle, not an ordinary
// operating business. Verified this doesn't false-positive on the metals/mining names already
// covered by sector matching: none of AU/BTG/KGC/HL/NEM/SCCO/FCX/AA/CLF ever had a revenue
// decline worse than -15% (their cyclicality shows up in cost/margin, not units×price, since
// metal prices there were rising through the sampled window) — so this is a genuinely different,
// complementary signal, not a redundant one. Also verified it doesn't fire on INCY (biotech):
// revenue rose every single year 2020-2025 despite its margin swinging on R&D/licensing timing,
// confirming the two are actually distinguishable by this test. -30% is deliberately well past
// even a bad recession year for a normal operating company.
// A commodity price cycle crashes AND recovers - MU's DRAM/NAND collapse rebounded from $15.5B
// back to $25.1B/$37.4B, well past the pre-crash $30.8B. A corporate restructuring (a spinoff or
// divestiture shrinking the reporting entity) also produces a one-year >=30% drop, but never
// recovers anywhere near the old level, since the business that generated that revenue no longer
// belongs to this entity going forward. Verified against real data: ADEA (Adeia Inc.)'s
// 2021-2022 revenue nearly halved ($877.7M to $438.9M) when Xperi's IP-licensing business spun
// out as the standalone Adeia entity - every year since has stayed near that new, smaller base
// (never exceeding ~50% of the pre-spinoff 2021 figure), the opposite shape of a commodity cycle
// riding demand/price back up. Without a rebound check this misread ADEA - an ordinary,
// non-cyclical patent-licensing business - as "cyclical commodity," applying the same stability
// haircut meant for MU/AU/HL to OPPO and GQS. Requiring at least one later year to recover to
// REVENUE_CRASH_REBOUND_THRESHOLD of the pre-crash level keeps catching MU while no longer
// flagging a permanent, one-time structural step-down as a price cycle.
const REVENUE_CRASH_REBOUND_THRESHOLD = 0.65;
function hasRevenueCrash(revHist, badYears) {
  const clean = (revHist || [])
    .filter(r => r.val > 0 && !badYears.has(r.year))
    .sort((a, b) => parseInt(a.year, 10) - parseInt(b.year, 10));
  for (let i = 1; i < clean.length; i++) {
    const preCrashLevel = clean[i - 1].val;
    if (clean[i].val / preCrashLevel - 1 <= -0.30) {
      const rebounds = clean.slice(i + 1).some(r => r.val >= preCrashLevel * REVENUE_CRASH_REBOUND_THRESHOLD);
      if (rebounds) return true;
    }
  }
  return false;
}

// Median YoY growth over a trailing window (default 5 years) of a {year, [valueKey]} series —
// windowed, not whole-history. computeRevenueGrowthMedian/data.epsCagr elsewhere in this file
// deliberately look at *all* available history (right choice for the DCF, where more data only
// helps), but that's wrong for a recent-stagnation check: a company with 10 years of history
// that grew fast in its first 5 and has flatlined for the last 5 gets a decade-blended median
// that still clears any reasonable hurdle, hiding exactly the stagnation this is meant to catch.
// Windowing to the trailing years only means only recent performance can rescue the score.
function windowedMedianGrowthPct(hist, valueKey, badYears, windowYears = 5) {
  const clean = (hist || [])
    .filter(h => h[valueKey] > 0 && !badYears.has(h.year))
    .sort((a, b) => parseInt(a.year, 10) - parseInt(b.year, 10));
  if (clean.length < 3) return null;
  const windowed = clean.slice(-(windowYears + 1));
  if (windowed.length < 3) return null;
  const yoy = [];
  for (let i = 1; i < windowed.length; i++) {
    const yrs = parseInt(windowed[i].year, 10) - parseInt(windowed[i - 1].year, 10);
    if (yrs > 0) yoy.push(Math.pow(windowed[i][valueKey] / windowed[i - 1][valueKey], 1 / yrs) - 1);
  }
  if (!yoy.length) return null;
  yoy.sort((a, b) => a - b);
  const mid = Math.floor(yoy.length / 2);
  const median = yoy.length % 2 ? yoy[mid] : (yoy[mid - 1] + yoy[mid]) / 2;
  return isFinite(median) ? median * 100 : null;
}

// Best available *recent* growth signal — trailing 3-year median revenue growth, windowed via
// windowedMedianGrowthPct above. Revenue only, deliberately: EPS growth is wildly convex right
// where it matters most for this check, near a small or recently-negative base — a company
// climbing out of a low-earnings year posts triple-digit "growth" every step of that climb with
// no real operating improvement behind most of it (verified against real data: YELP's EPS went
// -0.26 → 0.55 → 0.52 → 1.44 → 2.02 → 2.43 across 2020-2025, so *any* window touching that
// recovery reads as 40%+/yr "EPS growth" while revenue growth over the same stretch had already
// decelerated to mid-single digits — exactly the stagnation this hard cap exists to catch, which
// EPS would have completely hidden). Revenue has no such denominator problem — just units times
// price — which is the same reason computeRevenueGrowthMedian below prefers it over FCF/EPS for
// its own fallback. Three years, not five, for the same base-effect reason: YELP's 2020 revenue
// was itself COVID-depressed, so a 5-year window still anchors on that low base and reads as
// double-digit growth long after the real run-rate has settled back down to single digits.
function computeGrowthCagrPct(data) {
  const badYears = excludedYears(data.revHistory);
  return windowedMedianGrowthPct(data.revHistory, 'val', badYears, 3);
}

// The DCF's default dilution assumption (see computeDCFPointValue below), in place of the
// "shares never change for 10 years" that an unset dilutionPct silently assumed before.
// dilutionPct there is a flat, one-time multiplier on the final share count — the same
// *cumulative*-over-the-projection shape as the "Dilution Squeeze" stress grid's own 0-40% axis
// — so this reuses data.shareDilution (route.js: oldest-to-latest % change in diluted shares
// over the company's reported history) directly, rather than measuring an annual rate and
// compounding it out to 10 years, which would overstate things: a modest, real 6%/yr issuance
// pace compounds to ~80% over a decade, nearly double the stress grid's own designated worst
// case, if projected as a fresh compounding rate rather than anchored to what's actually been
// observed. Only ever pushes the default toward *more* dilution — floored at 0 so a company
// that's been shrinking its share count (buybacks outpacing SBC issuance, e.g. YELP: shares down
// ~20%) doesn't get a buyback-upside bonus baked into the base case — and capped at the stress
// grid's own 40% ceiling so this default can never exceed what the model itself calls severe.
function computeHistoricalDilutionPct(data) {
  if (data.shareDilution == null) return 0;
  return Math.max(0, Math.min(0.40, data.shareDilution / 100));
}
...
export function computeEasyMode(data, hasFundamentals) {
  ...
  // Metals & mining margins/ROIC swing hard with the underlying commodity price — unlike
  // energy (structurally thin margins, hence a *lower* threshold than default above), a
  // precious-metals miner's gross/operating margin at the top of a price cycle routinely
  // clears 60-90%/30-50%, well above the generic default (0.35/0.15) built for a normal
  // industrial. Verified against real data during a gold/silver upcycle: AU, BTG, KGC, HL
  // all scored 5/5 on both gmScore and omScore simultaneously — the generic bar was so far
  // below what any active miner clears in a good pricing environment that it stopped
  // discriminating between them at all. Raising the bar (not lowering it, the opposite of
  // energy) means only genuinely exceptional-for-the-sector margins still max out; see
  // computeStabilityFactor below for the complementary fix (the multi-year CV/trend check
  // that's supposed to catch a miner caught at its cyclical peak in the first place).
  const isMaterials = sector.includes('material') || sector.includes('metal') || sector.includes('mining')
    || industry.includes('metal') || industry.includes('mining');
  // REITs and regulated utilities carry debt, and run thin current ratios, as a normal part
  // of how those business models are financed — not as a risk signal the way it is for a
  // typical operating company. A REIT's real estate sits on the books at depreciated
  // historical cost while it's financed with non-recourse mortgage debt against the
  // property's market value, so book-equity D/E of 5-10x is routine for a healthy REIT, and
  // its balance sheet structurally has almost no "current" assets (the building isn't going
  // to convert to cash next quarter) — a low current ratio is universal across the sector,
  // not a discriminating signal. A regulated utility earns a return on a debt-financed rate
  // base by design and funds day-to-day operations against predictable, regulated revenue
  // rather than a working-capital buffer. Verified against real data: SPG and AMT — both
  // blue-chip, profitable REITs — scored deScore=1.00 (the worst possible score) purely from
  // net D/E of 5.7x and 10.6x, and every large utility sampled (AEP, AEE, ETR, CNP, PCG, DTE)
  // scored crScore under 2.2 purely from sub-1.0x current ratios that reflect their rate
  // structure, not distress — see deScore/crScore below for the sector-specific scales this
  // feeds.
  const isRealEstate = sector.includes('real estate') || sector.includes('reit') || industry.includes('reit');
  const isUtility = sector.includes('util');
  ...
  // Market-cap tier (lib/marketCap.js) scales the sector-relative thresholds below and the
  // Final Note weighting further down — hitting mega-cap-level margins/ROIC is partly a function
  // of scale itself, so a sub-$300M company clearing 80% of the "normal" bar reflects comparable
  // underlying execution to a mid-cap clearing the bar outright. Unknown market cap falls back to
  // the mid-cap tier, the file's implicit baseline before tiering existed.
  const capTier = getCapTier(data.marketCap) ?? DEFAULT_TIER;
  const roicThreshold = (isTech ? 0.25 : isPharma ? 0.20 : isConsumer ? 0.20 : isMaterials ? 0.18 : isEnergy ? 0.12 : 0.15) * capTier.thresholdScale;
  const gmThreshold = (isHardwareTech ? 0.40 : isTech ? 0.65 : isPharma ? 0.65 : isConsumer ? 0.45 : isFinancial ? 0.30 : isMaterials ? 0.55 : isEnergy ? 0.25 : 0.35) * capTier.thresholdScale;
  const omThreshold = (isTech ? 0.20 : isPharma ? 0.20 : isConsumer ? 0.15 : isFinancial ? 0.15 : isMaterials ? 0.25 : isEnergy ? 0.12 : 0.15) * capTier.thresholdScale;

  // data.roic (route.js) is already NOPAT over Assets − Current Liabilities — this used to
  // recompute a second, different ROIC here (NOPAT over Debt + Equity − Cash) as an "adjusted"
  // version, which just reintroduced the same failure mode route.js's own formula was fixed to
  // avoid: Debt+Equity collapses toward zero (or negative) for a debt-funded-buyback company
  // with negative equity, producing a misleadingly huge ROIC (verified against real data: DPZ
  // showed 100%+ this way vs. Finviz's reported 54.88%). Recomputing it here a second time, with
  // a worse denominator, also meant the Quality tab and the Financials tab could show two
  // different ROIC numbers for the same company. Falls back to Finnhub's TTM figure — already
  // merged into data.roic upstream — only when there's not enough statement detail for route.js
  // to have computed its own.
  const roicForScore = data.roic;
  ...
  //
  // Dividing by NEGATIVE equity (common for aggressive buyback programs that spend more than
  // the company has ever earned — verified against real data: DPZ, equity around -$4B against
  // ~$4.7B of real net debt from its securitized-notes-funded buybacks) flips the sign of this
  // ratio into territory the scale below reads as "less leveraged than zero debt":
  // interpolateScore clamps any value <= its lowest breakpoint (0) to the best score (5/5), and
  // the narrative sentence further down reads a negative value as "pristine net-cash position."
  // Both are backwards — negative equity from debt-funded buybacks is a real solvency signal,
  // not a strength, and both the score and the sentence read off this single computation.
  // Substituting a fixed high-end value (matching this table's own "5 → worst score" and
  // "2 → mid score" convention) whenever equity is negative fixes both at the same time,
  // instead of a misleadingly-negative ratio flowing downstream to either of them.
  const netDebtAmount = data.debtVal != null ? data.debtVal - (data.cashVal ?? 0) : null;
  const netDebtToEquity = data.equityVal != null && data.equityVal < 0
    ? (netDebtAmount != null && netDebtAmount <= 0 ? 2 : 5)
    : (data.debtVal != null && data.equityVal ? Math.max(0, data.debtVal - (data.cashVal ?? 0)) / data.equityVal : data.debtToEquity);
  // Net D/E scale shifted right for REITs (mortgage-financed real estate, routine 5-10x+ book
  // D/E) and utilities (regulated debt-financed rate base, routine ~1-2x) — see isRealEstate/
  // isUtility above for why the generic scale (calibrated for an ordinary operating company)
  // misreads normal leverage in these sectors as distress.
  const deScorePoints = isRealEstate ? [[0, 5], [2, 4.5], [4, 4], [6, 3], [9, 2], [14, 1]]
    : isUtility ? [[0, 5], [1, 4.5], [1.8, 4], [2.6, 3], [3.5, 2], [5, 1]]
    : [[0, 5], [0.7, 4.5], [1.2, 4], [2, 3], [3, 2], [5, 1]];
  const deScore = netDebtToEquity == null ? 2.5 : interpolateScore(netDebtToEquity, deScorePoints);
  ...
  //
  // REITs get a near-flat scale floored at 3, not 1 — real estate structurally isn't a
  // "current" asset, so every REIT in the sector reads near-zero here regardless of quality
  // (see isRealEstate above); this is "not applicable," not "risky," so it shouldn't drag the
  // score toward the worst case the way it would for an operating company. Utilities get a
  // gentler ramp reflecting that sub-1.0x is their normal operating range, same reasoning as
  // the Apple carve-out above, just shifted down a tier since regulated utilities run leaner
  // than a typical company but still meaningfully thinner than a REIT.
  ...
  const crScorePoints = isRealEstate ? [[0, 3], [0.3, 3.2], [1, 4], [3.33, 5]]
    : isUtility ? [[0, 2], [0.5, 3], [1, 4], [2, 4.5], [3.33, 5]]
    : [[0, 1], [1, 2], [3.33, 5]];
  const crScore = currentRatio == null ? 2.5 : interpolateScore(currentRatio, crScorePoints);
  ...
  // Dilution risk — data.shareDilution is oldest-to-latest % change in diluted share count
  // (route.js), reused here as a CBS ingredient rather than only feeding the DCF's default
  // dilution assumption (computeHistoricalDilutionPct below). Heavy historical dilution is a
  // direct cost to existing shareholders that ROIC/margins never see, since it shows up in the
  // share count, not the income statement — easy to paper over at small/micro scale behind
  // "the business is growing" while shareholders are quietly getting diluted away.
  const dilutionScore = data.shareDilution == null ? 2.5 : interpolateScore(data.shareDilution,
    [[-40, 5], [-10, 4.5], [0, 4], [15, 3], [30, 2], [60, 1]]);

  // Cash runway — only a risk when the company is actually burning cash; a company with
  // fcfVal >= 0 has no runway risk to score, so it gets full marks rather than the neutral 2.5
  // a "not applicable" null would otherwise imply. Uses raw fcfVal (real cash flow), not the
  // SBC-adjusted trueFcf below — SBC is non-cash, so it doesn't affect how long cash on hand
  // actually lasts. This is the one CBS ingredient the P/FCF-based OPPO score can't see:
  // pfcfScore/fcfYieldScore both floor to the same "1" whether a company is burning cash with
  // five years of runway or two quarters from insolvency — a critical distinction for small/
  // micro caps, where pre-profit businesses are common and mega caps almost never land here.
  const cashRunwayYears = data.cashVal != null && data.fcfVal != null && data.fcfVal < 0
    ? data.cashVal / Math.abs(data.fcfVal) : null;
  const runwayScore = data.fcfVal != null && data.fcfVal >= 0 ? 5
    : cashRunwayYears == null ? 2.5
    : interpolateScore(cashRunwayYears, [[0, 1], [0.5, 1.5], [1, 2.5], [2, 3.5], [4, 5]]);

  // "Skin in the game" — aggregate insider ownership from SEC Form 4 filings
  // (data.insiderOwnershipPct, computed in app/api/stock/route.js via lib/secInsiders.js). An
  // approximation by construction (only reflects insiders who've filed a Form 4 recently, not a
  // full beneficial-ownership register from a proxy statement), so it's treated gently: null
  // stays neutral rather than being punished as "zero ownership", and even a measured 0% only
  // costs a point and a half, not the full range.
  const ownershipScore = data.insiderOwnershipPct == null ? 2.5 : interpolateScore(data.insiderOwnershipPct,
    [[0, 1.5], [2, 2.5], [5, 3.5], [10, 4.5], [20, 5]]);

  // Stability multiplier — see computeStabilityFactor above. Applied to the raw weighted sum
  // before the usual 1-5 clamp, not after, so it can't be masked by a business that's already
  // sitting at the ceiling on every other CBS ingredient.
  // hasRevenueCrash (see above) catches commodity-priced businesses sector matching can't name
  // — memory-chip makers like MU report under the same generic "Semiconductors" sector/industry
  // as a differentiated logic/design chipmaker, with no field to tell them apart, but MU's own
  // revenue swung -49.5% then +62%/+49% on DRAM/NAND pricing alone, same shape as a metals miner
  // riding a commodity cycle.
  const isCyclicalCommodity = isMaterials || isEnergy || hasRevenueCrash(data.revHistory, excludedYears(data.revHistory));
  const { factor: stabilityFactor, marginCV, roicCV } = computeStabilityFactor(data, 7, capTier, isCyclicalCommodity);
  // OPPO/GQS only take the stability haircut for verified price-cyclical sectors — a commodity
  // miner's "cheap" P/FCF and "growth" revenue print are driven by the same price spike CBS's
  // margin/ROIC discount already accounts for, so leaving them undiscounted let the same
  // windfall inflate two more scores untouched (see isCyclicalCommodity above). Elsewhere
  // (biotech, tech, anything with lumpy-but-not-price-driven earnings) a P/FCF or revenue print
  // isn't reading a commodity price at all, so there's no matching justification to discount it.
  const oppoGqsStabilityFactor = isCyclicalCommodity ? stabilityFactor : 1;

  // CBS blends the five traditional fundamentals (ROIC/margins/leverage/liquidity) with a
  // "capital discipline" trio (dilution, cash runway, insider ownership) via
  // capTier.capitalDisciplineWeight (lib/marketCap.js) — barely relevant at mega-cap scale but
  // often decisive at small/micro. The five fundamentals split whatever weight capital
  // discipline doesn't use, equally — same 20%-each shape CBS always had, since
  // capitalDisciplineWeight defaults near 0 for mega/large.
  const fundamentalsWeight = (1 - capTier.capitalDisciplineWeight) / 5;
  const disciplineWeight = capTier.capitalDisciplineWeight / 3;
  const cbs = Math.min(5, Math.max(1, (
    gmScore * fundamentalsWeight + omScore * fundamentalsWeight + deScore * fundamentalsWeight
    + crScore * fundamentalsWeight + roicScore * fundamentalsWeight
    + dilutionScore * disciplineWeight + runwayScore * disciplineWeight + ownershipScore * disciplineWeight
    + (surplusCash != null && surplusCash > 0 ? 0.2 : 0)
  ) * stabilityFactor));

  // "True" FCF net of stock-based compensation. SBC is added back to CFO as a non-cash expense,
  // but it's a real dilution cost to existing shareholders — a P/FCF or FCF yield built on
  // unadjusted FCF reads a heavy-SBC business (mature tech especially) as cheaper and more
  // cash-generative than it actually is for a shareholder who doesn't get diluted away. Scoped to
  // this valuation block only: computeNormalizedFcf, computeReinvestmentGrowth and the DCF below
  // keep using the business's actual cash FCF, since that's what the company itself has on hand
  // to reinvest or return, dilution aside. Falls back to the unadjusted data.pfcf/data.fcfYield
  // when there's no sbcVal to net out.
  const trueFcf = data.fcfVal != null ? data.fcfVal - (data.sbcVal ?? 0) : null;
  const truePfcf = trueFcf != null && trueFcf > 0 && data.marketCap ? data.marketCap / trueFcf : data.pfcf;
  const trueFcfYield = trueFcf != null && data.marketCap ? (trueFcf / data.marketCap) * 100 : data.fcfYield;

  // null here means "no marketCap/FCF data to compute a ratio from" (e.g. a Finnhub outage
  // blanked marketCap for this fetch) — genuinely different from a computed truePfcf <= 0 (real,
  // priced-in negative or zero FCF, which does deserve the worst score) or a computed negative
  // trueFcfYield (already floored to near-1 by interpolateScore's own [-5, 1] anchor below).
  // Treating missing data as neutral, same convention every other CBS ingredient in this
  // function uses, keeps a data-pipeline gap from reading as "worst possible valuation" (verified
  // against real data: QCOM's marketCap dropping out from a Finnhub failure used to floor both
  // scores here to 1, cratering OPPO and the overall quality score to ~50 from a normal ~97 with
  // no actual change in the business or its valuation).
  const pfcfScore = truePfcf == null ? 2.5 : truePfcf <= 0 ? 1 : interpolateScore(truePfcf,
    [[0, 5], [12, 5], [18, 4.5], [25, 4], [35, 3], [50, 2], [80, 1]]);
  const fcfYieldScore = trueFcfYield == null ? 2.5 : interpolateScore(trueFcfYield,
    [[-5, 1], [0, 2], [1.5, 3], [3, 4], [5, 4.5], [8, 5]]);
  // Stability multiplier applied here too, not just to CBS above — a cyclical caught at its
  // margin peak (see computeStabilityFactor) has that same windfall FCF driving both scores
  // this blends: pfcfScore/fcfYieldScore read a commodity miner's peak-cycle cash flow as
  // "cheap," which reverses the moment the commodity price normalizes, exactly like the ROIC/
  // margin spike CBS already discounts. Floored at 1, same clamp cbs uses, so the multiplier
  // can't push an already-weak valuation score below the scale's own worst case.
  const oppo = Math.max(1, (pfcfScore*0.55 + fcfYieldScore*0.45) * oppoGqsStabilityFactor);

  // Multi-year growth signal (computeGrowthCagrPct above) in place of the raw single-year YoY
  // print, so a cyclical's peak year or a one-off easy comp doesn't read as sustained growth.
  // Falls back to the plain YoY figure when there isn't enough history for a CAGR.
  const growthCagrPct = computeGrowthCagrPct(data);
  // Reacceleration check: a flat median (the level signal above) is exactly the wrong tool for
  // telling "stagnant for years, no sign of turning" apart from "had a 2-year macro trough and
  // is already recovering" — both read as a low multi-year median, since a median by
  // construction discounts the one recent point that actually matters for a forward-looking
  // model (verified against real data: AAPL's revenue was flat-to-down 2022-2024, then +6.4% in
  // 2025 — the 3yr median reads 2.0%, same order as a genuinely stagnant name, even though the
  // latest year is already well above that median). data.revGrowth is the plain latest-year YoY,
  // already computed upstream — no new data needed.
  const isReaccelerating = growthCagrPct != null && data.revGrowth != null && data.revGrowth > growthCagrPct;
  // Blend the trailing median with the latest print both ways, not just on the upside — the old
  // "below the median, leave the level signal alone" behavior let a severe, current slowdown hide
  // entirely behind a stale multi-year median (verified against real data: QCOM's 3yr median sat
  // around +9% off FY24/FY25's recovery while its latest TTM print had decelerated to +0.9%, and
  // the score — 81/100 — reflected only the +9% trend, as if the slowdown hadn't happened). A
  // straight 50/50 blend in both directions keeps a single mild dip from swinging the score much
  // (small gap in, small move out) while still letting a sharp, real deceleration pull it down.
  const effectiveGrowthPct = growthCagrPct != null && data.revGrowth != null
    ? (growthCagrPct + data.revGrowth) / 2
    : (growthCagrPct ?? data.revGrowth);
  const revGrowthInput = effectiveGrowthPct ?? data.revGrowth;
  const revGrowthScore = revGrowthInput == null ? 2.5 : interpolateScore(revGrowthInput,
  ...
  const gqsRaw = Math.min(5, Math.max(1, roicScore * 0.5 + revGrowthScore * 0.5 + rdBonus + sbcPenalty + lossPenalty));
  // Smooth growth ceiling in place of the old hard cliff (jumping straight to 40/100 the instant
  // growth dipped under the hurdle, with no regard for how far under, or how strong the other
  // GQS ingredients were) — same reasoning as interpolateScore up top: a company sitting just
  // under a cutoff shouldn't score identically to one sitting well below it. Two different
  // curves, not one, because the *reason* a company is under the hurdle changes how forgiving
  // this should be: a name that's decelerating with no sign of a turn (YELP: latest year still
  // below its own trailing trend) gets the steep curve — near-identical bite to the old flat 40
  // cap through the whole realistic "modest but real growth" zone (verified: this keeps YELP's
  // score within a point of where the flat cap left it). A name mid-recovery from a temporary
  // trough (AAPL: latest year already back above trend) gets the gentle curve — real credit for
  // the turn already underway, without full marks, since the *level* is still genuinely modest.
  // Both flatten below/above their end anchors, same as interpolateScore always does.
  const growthCeilingPoints = isReaccelerating
    ? [[0, 3.0], [7, 4.5], [15, 5]]
    : [[-5, 1.2], [0, 1.8], [7, 2.2], [15, 5]];
  const growthCeilingScore = effectiveGrowthPct == null ? 5 : interpolateScore(effectiveGrowthPct, growthCeilingPoints);
  // Same stability multiplier as cbs/oppo above — revGrowthScore (half of gqsRaw) is exactly
  // as inflated by a commodity price spike as the margin/ROIC figures CBS already discounts:
  // a miner's revenue "growth" during a price upcycle is the price moving, not units sold or
  // market share gained, so it deserves the same cyclicality haircut. Floored at 1.
  const gqs = Math.max(1, Math.min(gqsRaw, growthCeilingScore) * oppoGqsStabilityFactor);
  ...
  // Blend weights come from the cap tier (lib/marketCap.js): mega/large caps are judged mostly
  // on demonstrated capital stewardship (CBS), since their growth optionality is inherently
  // limited by size; small/micro caps lean on growth trajectory (GQS), since that's the actual
  // investment thesis at that stage and CBS's stability-heavy ingredients would otherwise punish
  // every young company for not yet being a mega-cap.
  const { cbs: cbsWeight, oppo: oppoWeight, gqs: gqsWeight } = capTier.weights;
  const rawFinalNote = cbs*cbsWeight + oppo*oppoWeight + gqs*gqsWeight + moatBonus;
  ...
  if (data.revGrowth != null && data.fcfVal != null && data.debtToEquity != null) {
    const companyName = data.name?.split(' ')[0]?.replace(/[.,\/#!$%\^&\*;:{}=\-_`~()]/g, "") || 'This company';

    // 1. Competitive Moat prefix
    let moatPrefix = '';
    if (moat === 'WIDE') {
      moatPrefix = 'Supported by a wide competitive moat, ';
    } else if (moat === 'NARROW') {
      moatPrefix = 'With a narrow competitive moat, ';
    }

    // 2. Growth description
    let growthText = '';
    const g = data.revGrowth;
    if (g > 15) {
      growthText = `is growing rapidly at ${g.toFixed(0)}% annually`;
    } else if (g > 5) {
      growthText = `expands revenue steadily at ${g.toFixed(0)}% per year`;
    } else if (g > 0) {
      growthText = `grows revenue slowly (${g.toFixed(0)}% YoY)`;
    } else {
      growthText = `faces sales contraction (revenue down ${Math.abs(g).toFixed(0)}% YoY)`;
    }

    // 3. Margin & Cash Flow description
    let cashText = '';
    if (data.fcfVal > 0) {
      if (data.opMargin && data.opMargin > 20) {
        cashText = 'while maintaining exceptionally high operating margins';
      } else {
        cashText = 'while generating healthy, positive free cash flow';
      }
    } else {
      cashText = 'but is currently burning cash';
    }

    // 4. Financial Health & Debt description
    let debtText = '';
    const de = netDebtToEquity ?? data.debtToEquity;
    if (de < 0) {
      debtText = 'backed by a pristine net-cash position';
    } else if (de < 0.6) {
      debtText = 'backed by an extremely clean balance sheet';
    } else if (de < 1.2) {
      debtText = 'with a highly manageable debt profile';
    } else {
      debtText = 'though it carries a heavily leveraged balance sheet';
    }

    // Combine into a beautiful natural sentence
    const rawSentence = `${moatPrefix}${companyName} ${growthText} ${cashText}, ${debtText}.`;
    summary = rawSentence.charAt(0).toUpperCase() + rawSentence.slice(1);
  } else {
    summary = `Not enough data yet to establish a comprehensive quality rating for ${data.name || 'this company'}.`;
  }

  return {
    score100, verdict, verdictColor, summary,
    cbs, oppo, gqs, finalNote, moat, moatColor, capTier,
    ...
    growthCagrPct, gqsRaw, trueFcf, truePfcf, trueFcfYield, stabilityFactor, marginCV, roicCV,
    dilutionScore, runwayScore, ownershipScore, cashRunwayYears,
    shareDilution: data.shareDilution, insiderOwnershipPct: data.insiderOwnershipPct,
  };
}
...
// --- Relative valuation, quality-adjusted — replaces the forward 10-year DCF as the primary
// valuation. ---
//
// The forward DCF stacked 4-5 independently uncertain assumptions (WACC via beta, a decade of
// reinvestment×ROIC growth, a margin-recovery ramp, a terminal exit multiple) that compound
// multiplicatively over 10 years — a small error in any one of them swings the output wildly,
// a well-known property of the technique itself, not a bug specific to this file (verified
// against real data: even after independently fixing three separate issues in that stack,
// YELP's "fair value" only moved from 594% to 432% upside — patching individual assumptions one
// at a time kept producing indefensible numbers). Replaced with two much simpler, more
// auditable pieces:
//   (A) Reverse-DCF: solve for the single growth rate today's price already implies
//       (computeImpliedGrowth below), instead of projecting one forward — one assumption (WACC)
//       instead of four, and the output is a growth rate a person can sanity-check directly
//       against the company's own reported history ("does 15%/yr forever seem right for a
//       company that's grown 4%/yr for a decade?"), unlike a WACC/margin/multiple combination
//       nobody can eyeball.
//   (B) A target P/FCF multiple blended between the company's own *current* multiple and its
//       sector's typical one — not full assumed reversion to the sector norm the way the old
//       exit multiple was. "Current" rather than "own historical average" because this pipeline
//       has no historical market-cap series to compute the company's own *past* multiples from
//       (only current price/shares, not price at each past fiscal year-end) — current is the
//       honest anchor actually available. The blend weight comes from this company's own
//       Quality Score (CBS/GQS, already computed by computeEasyMode) — a business with real,
//       durable growth quality earns more credit for reverting toward the sector's typical
//       multiple; one that's profitable but not growing (YELP) stays anchored near where the
//       market has already been pricing it, instead of assuming an unearned re-rating.
function computeBusinessQualityWeight(easyMode) {
  if (!easyMode) return 0;
  const cbsNorm = Math.max(0, Math.min(1, (easyMode.cbs - 1) / 4));
  const gqsNorm = Math.max(0, Math.min(1, (easyMode.gqs - 1) / 4));
  // Growth quality weighted more than core-business health — a healthy balance sheet doesn't by
  // itself justify assuming the market will re-rate the stock; durable growth does.
  return cbsNorm * 0.3 + gqsNorm * 0.7;
}

export function computeRelativeValue(data, easyMode) {
  if (!easyMode || !data.marketCap || !data.currentPrice) return null;
  const trueFcf = easyMode.trueFcf;
  if (trueFcf == null || trueFcf <= 0) return null;
  // Derived from marketCap/currentPrice when the direct field is missing (verified against real
  // data: Visa's own fetch has marketCap and currentPrice populated but sharesOutstanding null) —
  // mathematically the same number, since marketCap is itself price × shares upstream.
  const shares = data.sharesOutstanding ?? data.marketCap / data.currentPrice;
  if (!shares) return null;

  const fcfPerShare = trueFcf / shares;
  const sectorMultiple = computeNormalizedExitMultiple(data.sector, data.industry);
  // Capped at 3x the sector multiple so a temporary near-zero-FCF quarter (which can send a raw
  // P/FCF to triple digits) can't anchor the blend on a meaningless multiple.
  const ownMultipleRaw = easyMode.truePfcf != null && easyMode.truePfcf > 0 ? easyMode.truePfcf : null;
  const ownMultiple = ownMultipleRaw != null ? Math.min(ownMultipleRaw, sectorMultiple * 3) : null;
  const qualityWeight = computeBusinessQualityWeight(easyMode);

  // Direction-aware: quality determines how much the model trusts the market's *current*
  // pricing of this business, whichever direction it's priced in — not just "credit for
  // reverting up toward sector normal." A high-quality business trading *above* sector typical
  // earns credit for that premium being deserved (real ROIC/moat/growth, not just hype); a
  // high-quality business trading *below* typical earns credit for reverting up toward it (the
  // case this was originally built for). Either way, low quality pulls the target toward the
  // neutral sector benchmark, not toward whatever the market happens to be doing right now.
  // Without this, a stock trading above the sector multiple for genuinely good reasons (real
  // data: MSFT's own 50x vs. tech's 24x during the AI-driven re-rating) reads as "quality
  // pulls it down toward sector normal" — treating the market's own premium as automatically
  // suspect regardless of how strong the underlying business actually is.
  const blendMultiple = (weight) => {
    if (ownMultiple == null) return sectorMultiple;
    return ownMultiple > sectorMultiple
      ? ownMultiple * weight + sectorMultiple * (1 - weight)
      : ownMultiple * (1 - weight) + sectorMultiple * weight;
  };

  const targetMultiple = blendMultiple(qualityWeight);
  const fairValue = targetMultiple * fcfPerShare;

  // Bear/bull band width scales with this company's own historical earnings volatility (margin/
  // ROIC coefficient of variation - already computed by computeStabilityFactor for CBS/OPPO/GQS,
  // reused here rather than a fresh invented signal) instead of re-blending ownMultiple/
  // sectorMultiple at a different weight. That weight-only approach bounded the entire bear-to-
  // bull spread to |ownMultiple - sectorMultiple| - whenever a stock's own multiple already sits
  // close to its sector's (an unremarkable, common case, not a sign of low uncertainty), the
  // range was mathematically incapable of spreading no matter how far the weight swung. Verified
  // against real data: ADEA's own P/FCF (23.5x) sits within 2% of its sector's typical multiple
  // (24x), so the old approach produced a bear-to-bull spread of just 1.3% ($26.26-$26.61) - not
  // a usable "pessimistic vs optimistic" scenario by any standard, regardless of how the
  // underlying business quality scored. A direct, symmetric percentage band on the single target
  // multiple has no such degenerate case. Bounded to [5%, 25%] so a very stable business still
  // shows a visible range and a genuinely volatile one doesn't blow out to an unusable spread.
  const uncertaintyPct = Math.min(0.25, Math.max(0.05, Math.max(easyMode.marginCV ?? 0, easyMode.roicCV ?? 0) * 0.3));
  const bearValue = targetMultiple * (1 - uncertaintyPct) * fcfPerShare;
  const bullValue = targetMultiple * (1 + uncertaintyPct) * fcfPerShare;

  // Reverse-DCF context (Part A) — the single-assumption sanity check, kept separate from the
  // fair-value figure above so a WACC error can no longer distort the headline number the way it
  // did when WACC drove a 10-year compounding projection.
  const wacc = computeWACC(data, data.riskFreeRate);
  const impliedGrowthPct = wacc != null ? computeImpliedGrowth(data.marketCap, trueFcf, wacc) * 100 : null;
  const realGrowthPct = easyMode.growthCagrPct ?? data.revGrowth ?? null;

  return {
    fairValue, bearValue, bullValue,
    targetMultiple, ownMultiple, sectorMultiple, qualityWeight, uncertaintyPct,
    fcfPerShare, wacc, impliedGrowthPct, realGrowthPct,
  };
}
...
export function computeDCFPointValue(data, riskFreeRate, overrides = {}) {
  ...
  // Defaults to the company's own trailing dilution trend (computeHistoricalDilutionPct above)
  // instead of assuming share count never changes — a caller (the "Dilution Squeeze" stress
  // grid) can still pin this to any explicit value, 0 included.
  const dilutionPct = overrides.dilutionPct ?? computeHistoricalDilutionPct(data);
...
export function computeDCFValue(data, riskFreeRate) {
  ...
  // A margin that just finished a rapid, multi-year re-rate (see marginRecentlyMatured) is
  // real today but hasn't been *proven durable* the way a margin that's been stable for years
  // has — competitors, pricing pressure, or reversion could still erode it before the terminal
  // year. The terminal value already gets the full benefit of that margin through fcfBase and
  // baseGrowth; this haircut only tempers how much confidence the *multiple* itself places on
  // that margin holding for a decade, rather than assuming day-one durability. 25% is a
  // deliberately blunt, conservative-leaning discount, not a fitted number.
  const marginJustMatured = marginRecentlyMatured(data);
  const exitMultiple = computeNormalizedExitMultiple(data.sector, data.industry) * (marginJustMatured ? 0.75 : 1);
  ...
  // Resolved once here (see computeHistoricalDilutionPct) and passed explicitly to every
  // scenario below, so bear/base/bull all price in the same dilution assumption instead of
  // each silently recomputing it — and so it's available on the returned object for the UI to
  // disclose alongside wacc/growth.
  const dilutionPct = computeHistoricalDilutionPct(data);
  ...
  const buildScenario = (key, label, waccDelta, growthDelta, primary) => {
    ...
    const value = +computeDCFPointValue(data, riskFreeRate, {
      wacc: scenarioWacc, growth: scenarioGrowth, exitMultiple, fcfBase, plateauYears, matureFcfMargin, fcfMarginRampYears, dilutionPct,
    }).toFixed(2);
  ...
  // A DCF this far above the current quote is possible (a genuinely mispriced stock exists),
  // but it's also exactly the signature of a modeling blind spot — surfaced as data, not
  // suppressed, so the UI can visibly caveat it instead of presenting an extreme number with
  // the same confidence as an ordinary one. Terminal-value-dominated names (SaaS margin
  // re-rates, cyclical troughs) are the recurring case in practice.
  const baseValue = scenarios.find(s => s.primary)?.value;
  const upsideVsPrice = data.currentPrice > 0 && baseValue != null ? (baseValue - data.currentPrice) / data.currentPrice : null;
  const extremeUpside = upsideVsPrice != null && upsideVsPrice > 1.5;

  return {
    wacc, baseGrowth, exitMultiple, impliedGrowth,
    normalizedFcf, latestFcf, reinvestmentPhase, marginDepressed, marginJustMatured, fcfBaseMethod, fcfBase,
    plateauYears, matureFcfMargin, fcfMarginRampYears, extremeUpside, dilutionPct,
    scenarios,
  };
}
...
// `estimate` is always positive when present — computeBulltraceValue already refuses to
