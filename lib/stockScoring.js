...
export function computeEasyMode(data, hasFundamentals) {
  ...
  const isBlueGem = rawFinalNote > 5;
...
export function computeReinvestmentGrowth(data) {
  ...
  const years = Object.keys(oi).filter(y =>
    ebt[y] != null && tax[y] != null && capex[y] != null && da[y] != null && debt[y] != null && equity[y] != null
  );
  ...
  for (const y of years) {
    ...
    const investedCapital = debt[y] + equity[y] + rdAsset;
...
// computeReinvestmentGrowth above), falling back to FCF growth over the reported history,
// then latest YoY revenue growth, then a flat conservative default.
