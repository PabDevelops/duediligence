...
export function computeEasyMode(data, hasFundamentals) {
  ...
  const revGrowthInput = growthCagrPct ?? data.revGrowth;
  ...
  // Hard cap: below a 7%/yr absolute growth hurdle — real terms, not sector-relative, since a
  // stagnant sector doesn't excuse a stagnant company — GQS can't clear 40/100 (2.0 on this 1-5
  // scale) no matter how well the other GQS ingredients score. Only fires when we actually have
  // a multi-year figure to judge against; a lone YoY print isn't enough evidence either way.
  const GROWTH_HURDLE_PCT = 7;
  const gqs = growthCagrPct != null && growthCagrPct < GROWTH_HURDLE_PCT ? Math.min(gqsRaw, 2.0) : gqsRaw;
