...
function computeStabilityFactor(data, years = 7, capTier = DEFAULT_TIER) {
  ...
  let factor = 1;
  if (worstCV > CV_THRESHOLD) factor -= Math.min(0.15, (worstCV - CV_THRESHOLD) * 0.3);
  if (isDeclining) factor -= 0.05;
  return { factor: Math.max(0.85, factor), marginCV, roicCV };
}
...
export function computeEasyMode(data, hasFundamentals) {
  ...
  const roicThreshold = (isTech ? 0.25 : isPharma ? 0.20 : isConsumer ? 0.20 : isEnergy ? 0.12 : 0.15) * capTier.thresholdScale;
  const gmThreshold = (isHardwareTech ? 0.40 : isTech ? 0.65 : isPharma ? 0.65 : isConsumer ? 0.45 : isFinancial ? 0.30 : isEnergy ? 0.25 : 0.35) * capTier.thresholdScale;
  const omThreshold = (isTech ? 0.20 : isPharma ? 0.20 : isConsumer ? 0.15 : isFinancial ? 0.15 : isEnergy ? 0.12 : 0.15) * capTier.thresholdScale;
  ...
  const gqs = Math.min(gqsRaw, growthCeilingScore);
