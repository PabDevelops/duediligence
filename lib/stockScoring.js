...
function computeStabilityFactor(data, years = 7, capTier = DEFAULT_TIER) {
  ...
  let factor = 1;
  // No ceiling on the CV-based discount itself (previously capped at Math.min(0.15, ...),
  // so a mildly-above-threshold CV and a wildly cyclical one — a commodity miner's margin/
  // ROIC riding a price spike, CV well over 1.0 — got the identical, token 15% haircut.
  // Verified against real data: HL's marginCV/roicCV both topped 1.2 (over 4x the large-cap
  // threshold) yet still landed at the same floor as a stock barely past the threshold. The
  // overall floor below still stops this from ever fully zeroing out a score on volatility
  // alone.
  if (worstCV > CV_THRESHOLD) factor -= (worstCV - CV_THRESHOLD) * 0.3;
  if (isDeclining) factor -= 0.05;
  return { factor: Math.max(0.5, factor), marginCV, roicCV };
}
...
export function computeEasyMode(data, hasFundamentals) {
  ...
  const { factor: stabilityFactor, marginCV, roicCV } = computeStabilityFactor(data, 7, capTier);
  ...
  const oppo = Math.max(1, (pfcfScore*0.55 + fcfYieldScore*0.45) * stabilityFactor);
  ...
  const gqs = Math.max(1, Math.min(gqsRaw, growthCeilingScore) * stabilityFactor);
