...
function hasRevenueCrash(revHist, badYears) {
  ...
  for (let i = 1; i < clean.length; i++) {
    if (clean[i].val / clean[i - 1].val - 1 <= -0.30) return true;
  }
  return false;
}
...
export function computeRelativeValue(data, easyMode) {
  ...
  // A modest range around the base case by varying how much credit the quality weight itself
  // earns — not a second, independent uncertain assumption stacked on top (there's only ever
  // one real lever here: how much this company's own quality supports reverting toward the
  // sector multiple), so the range stays proportionate instead of compounding the way the old
  // bear/base/bull scenarios did.
  const bearValue = blendMultiple(qualityWeight * 0.5) * fcfPerShare;
  const bullValue = blendMultiple(Math.min(1, qualityWeight * 1.5)) * fcfPerShare;
  ...
  return {
    ...
    targetMultiple, ownMultiple, sectorMultiple, qualityWeight,
