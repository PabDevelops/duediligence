...
function sectorWaccRange(sector) {
  ...
  if (s.includes('retail') || s.includes('consumer') || s.includes('food') || s.includes('beverage')) return SECTOR_WACC_RANGE.staples;
...
export function computeWACC(data, riskFreeRate) {
  ...
  const [min, max] = sectorWaccRange(data.sector);
...
// needs a growth rate that comes from the business, not from its own quote: FCF growth over
// the reported history when there's enough of it, falling back to latest YoY revenue
// growth, then a flat conservative default when neither is available.
export function computeFundamentalGrowth(data) {
  const hist = (data.fcfHistory || []).filter(h => h.val > 0).sort((a, b) => parseInt(a.year, 10) - parseInt(b.year, 10));
  if (hist.length >= 3) {
...
// missing.
const SECTOR_EXIT_MULTIPLES = { tech: 24, pharma: 20, staples: 19, discountRetail: 13, restaurant: 16, financial: 13, energy: 11, default: 16 };
...
export function computeDCFPointValue(data, riskFreeRate, overrides = {}) {
  ...
  for (let year = 1; year <= 10; year++) {
    const yearGrowth = growth - (growth - terminalGrowth) * (year - 1) / 9;
...
export function computeDCFValue(data, riskFreeRate) {
  ...
  const impliedGrowth = computeImpliedGrowth(data.marketCap, data.fcfVal, wacc);
...
// stock — no equity compounds a near-term growth-stock rate for a full decade. The
// projection's drift fades the same way, down to a generic long-run equity-market return
// (risk-free rate + the same equity risk premium CAPM/WACC already use above) by this many
// years out, then holds flat.
const PROJECTION_FADE_YEARS = 10;

// Average of the linearly-fading instantaneous drift over [0, t] — needed (rather than a
// single flat number) because both the analytic percentile band and the simulated paths
// below accumulate drift over time, and a point at t=1yr should barely be faded while a
// point at t=10yr should be pulled most of the way to the terminal rate.
function averageProjectionDrift(drift0, terminalDrift, t) {
  if (t <= 0) return drift0;
  if (t >= PROJECTION_FADE_YEARS) {
    const fadeAvg = (drift0 + terminalDrift) / 2;
    return (fadeAvg * PROJECTION_FADE_YEARS + terminalDrift * (t - PROJECTION_FADE_YEARS)) / t;
  }
  return drift0 - (drift0 - terminalDrift) * (t / (2 * PROJECTION_FADE_YEARS));
}
...
export function computeProjection({ price, driftAnnual, volAnnual, horizonYears, stepsPerYear = 52, rng = Math.random, numPaths = 18, riskFreeRate = 0.045 }) {
  ...
  const drift0 = driftAnnual, vol = volAnnual;
  const terminalDrift = riskFreeRate + EQUITY_RISK_PREMIUM;
  ...
  const paths = Array.from({ length: numPaths }, () => {
    ...
    for (let i = 1; i <= totalSteps; i++) {
      ...
      // Midpoint of this step's interval, so the per-step drift already reflects the fade
      // instead of only the closed-form band below reflecting it.
      const stepDrift = averageProjectionDrift(drift0, terminalDrift, (i - 0.5) * dt);
      simPrice = simPrice * Math.exp((stepDrift - 0.5 * vol * vol) * dt + vol * Math.sqrt(dt) * z);
      path.push(simPrice);
    }
    return path;
  });
  ...
  for (let i = 1; i <= totalSteps; i++) {
    const t = i * dt;
    const avgDrift = averageProjectionDrift(drift0, terminalDrift, t);
    const driftTerm = (avgDrift - 0.5 * vol * vol) * t;
