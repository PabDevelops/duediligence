...
export function initializeScheduler() {
  ...
  // Snapshot the true closing price/% for whatever's currently showing in Home's movers
  // lists, right after the close, instead of leaving that snapshot to whenever a real user
  // next happens to view one of those tickers (see currentTradingDayBoundaryUtc in
  // lib/marketStatus.js and the comment in /api/movers). Uses node-cron's `timezone` option
  // so 4:05pm ET is correct year-round regardless of the server's own timezone or DST.
  cron.schedule('5 16 * * 1-5', async () => {
    console.log('[Scheduler] Running refresh-movers-close...');
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/api/admin/refresh-movers-close`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-Cron-Secret': process.env.CRON_SECRET || '',
      ...
      const result = await response.json();
      console.log('[Scheduler] Refresh-movers-close result:', result);
    } catch (error) {
      console.error('[Scheduler] Error running refresh-movers-close:', error);
    }
  }, { timezone: 'America/New_York' });

  console.log('[Scheduler] Initialized: auto-post-educational at 8:03 AM daily, seed-earnings-calendar at 3:00 AM daily, seed-tickers at 6:30 AM daily, refresh-movers-close at 4:05 PM ET on weekdays');
}
