...
export async function getYahooAuth() {
  ...
  if (!cookie) {
    try {
      ...
      cookie = (r1b.headers.get('set-cookie') || '').split(';')[0];
    } catch (e) {}
  }

  try {
    const r2 = await fetch('https://query1.finance.yahoo.com/v1/test/getcrumb', {
      headers: { 'User-Agent': UA, Cookie: cookie, Origin: 'https://finance.yahoo.com' },
    });
    ...
    console.error('[yahooAuth] failed:', err.message);
    return { cookie: '', crumb: '' };
  }
}
...
// Analyst consensus target price (financialData.targetMeanPrice) as an independent, lightweight
// fetch — NOT bundled into fetchYahooFundamentals's full statement pull. The main /api/stock
// route only calls fetchYahooFundamentals as a backfill when SEC EDGAR data is thin, so for any
// well-covered ticker (AAPL, MSFT, ...) that full fetch — and the target price living inside it —
// never ran at all. Target price has no SEC EDGAR equivalent, so it needs its own always-on path
// regardless of how good the SEC statement data already is.
export async function fetchYahooAnalystTarget(ticker) {
  try {
    const auth = await getYahooAuth();
    const res = await fetch(
      `https://query2.finance.yahoo.com/v10/finance/quoteSummary/${ticker}?modules=financialData,summaryDetail&crumb=${encodeURIComponent(auth.crumb)}`,
      { headers: { 'User-Agent': 'Mozilla/5.0', Cookie: auth.cookie } }
    );
    const json = await res.json();
    const r = json?.quoteSummary?.result?.[0];
    let target = r?.financialData?.targetMeanPrice?.raw;
    if (typeof target !== 'number') return null;
    // Same GBp (pence) → GBP fix as fetchYahooFundamentals's own analystTarget handling.
    if (r?.summaryDetail?.currency === 'GBp') target /= 100;
    return target;
  } catch {
    return null;
  }
