...
export default function MarketBar() {
  ...
  if (!markets.length) return (
    <div style={{ borderBottom: '1px solid var(--border)', padding: '8px 24px', display: 'flex', gap: '32px', background: 'var(--bg-1)' }}>
  ...
  return (
    <div style={{ borderBottom: '1px solid var(--border)', padding: '8px 24px', display: 'flex', justifyContent: 'space-between', background: 'var(--bg-1)', overflowX: 'auto' }}>
      {markets.map(m => {
        ...
        return (
          <div key={m.symbol} style={{ display: 'flex', alignItems: 'center', gap: '12px', flexShrink: 0 }}>
            <div>
